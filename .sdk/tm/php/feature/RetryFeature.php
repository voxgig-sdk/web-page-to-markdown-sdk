...
// WebPageToMarkdown SDK retry feature
...
class WebPageToMarkdownRetryFeature extends WebPageToMarkdownBaseFeature
{
    ...
    public function init(WebPageToMarkdownContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WebPageToMarkdownContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WebPageToMarkdownContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WebPageToMarkdownContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
