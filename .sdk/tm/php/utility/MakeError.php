...
class WebPageToMarkdownMakeError
{
    public static function call(?WebPageToMarkdownContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WebPageToMarkdownContext([], null);
        }
        $op = $ctx->op ?? new WebPageToMarkdownOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WebPageToMarkdownResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WebPageToMarkdownError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WebPageToMarkdownError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof WebPageToMarkdownError) {
