...
module WebPageToMarkdownUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WebPageToMarkdownResponse.new({})
    ...
    if fetch_err
      ...
      response = WebPageToMarkdownResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WebPageToMarkdownResponse.new(fetched)
    else
