...
boru-driven command-line client **and** interactive REPL for the WebPageToMarkdown
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/web-page-to-markdown-cli)
make build

# 2. See usage (words, entities, env vars)
./web-page-to-markdown-cli --help

# 3. Provide credentials once, via the environment
export WEB_PAGE_TO_MARKDOWN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./web-page-to-markdown-cli load 1 convert_url_to_markdown_get            # {id:1} shorthand
./web-page-to-markdown-cli load '{id:1}' convert_url_to_markdown_get       # explicit match map

# 5. Override the API base URL for a single call
WEB_PAGE_TO_MARKDOWN_BASE=https://api.example.com ./web-page-to-markdown-cli load 1 convert_url_to_markdown_get

# 6. No arguments -> interactive REPL
./web-page-to-markdown-cli
web-page-to-markdown> load 1 convert_url_to_markdown_get
web-page-to-markdown> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/web-page-to-markdown-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/web-page-to-markdown-cli load 1 convert_url_to_markdown_get
   ```
...
```sh
./web-page-to-markdown-cli load 1 convert_url_to_markdown_get          # scalar shorthand for {id:1}
./web-page-to-markdown-cli load '{id:1}' convert_url_to_markdown_get     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `web-page-to-markdown>`). Each line is
evaluated as its own boru expression:

```text
$ ./web-page-to-markdown-cli
web-page-to-markdown> load 1 convert_url_to_markdown_get
web-page-to-markdown> /help
web-page-to-markdown> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `convert_url_to_markdown_get`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/web-page-to-markdown-cli`. |
...
(`./web-page-to-markdown-cli <expr>`) and interactively (the REPL), and expressions compose the
