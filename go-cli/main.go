// Command WebPageToMarkdown-cli is a CLI + REPL for the WebPageToMarkdown SDK,
...
const prompt = "web-page-to-markdown"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "convert_url_to_markdown_get convert_url_to_markdown_post"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WEB_PAGE_TO_MARKDOWN_APIKEY (api key), WEB_PAGE_TO_MARKDOWN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WEB_PAGE_TO_MARKDOWN_APIKEY carries the API key and
	// WEB_PAGE_TO_MARKDOWN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WEB_PAGE_TO_MARKDOWN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WEB_PAGE_TO_MARKDOWN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWebPageToMarkdownSDK(opts)
