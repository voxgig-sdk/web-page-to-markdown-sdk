# web-page-to-markdown-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WebPageToMarkdown SDK as
two agent tools — `web-page-to-markdown_list` and `web-page-to-markdown_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/web-page-to-markdown-mcp)
make build

# 2. Provide credentials via the environment
export WEB_PAGE_TO_MARKDOWN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user web-page-to-markdown \
  -- /absolute/path/to/web-page-to-markdown-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./web-page-to-markdown-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// web-page-to-markdown_list: first page of records
{ "entity": "convert_url_to_markdown_get" }
{ "entity": "convert_url_to_markdown_get", "query": { } }

// web-page-to-markdown_load: one record by id
{ "entity": "convert_url_to_markdown_get", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/web-page-to-markdown-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user web-page-to-markdown \
     -- "$PWD"/dist/*/web-page-to-markdown-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `web-page-to-markdown_list` and `web-page-to-markdown_load` tools now appear
   in new sessions. Ask the agent to *"list convert_url_to_markdown_get using web-page-to-markdown"*
   and it calls `web-page-to-markdown_list` with `{"entity":"convert_url_to_markdown_get"}`.
...
```sh
export WEB_PAGE_TO_MARKDOWN_APIKEY=sk_live_xxx            # API key
export WEB_PAGE_TO_MARKDOWN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `web-page-to-markdown_list` tool
...
### Call the `web-page-to-markdown_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `web-page-to-markdown_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `web-page-to-markdown_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEB_PAGE_TO_MARKDOWN_APIKEY` | API key sent with every request. |
| `WEB_PAGE_TO_MARKDOWN_BASE` | Optional override of the API base URL. |
...
```sh
./web-page-to-markdown-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"web-page-to-markdown_load","arguments":{"entity":"convert_url_to_markdown_get","query":{"id":1}}}}'
```
