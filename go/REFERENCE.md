# WebPageToMarkdown Golang SDK Reference

Complete API reference for the WebPageToMarkdown Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *WebPageToMarkdownSDK`
...
#### `ConvertUrlToMarkdownGet(data map[string]any) WebPageToMarkdownEntity`

Create a new `ConvertUrlToMarkdownGet` entity instance. Pass `nil` for no initial data.

#### `ConvertUrlToMarkdownPost(data map[string]any) WebPageToMarkdownEntity`

Create a new `ConvertUrlToMarkdownPost` entity instance. Pass `nil` for no initial data.
...
```go
result, err := client.ConvertUrlToMarkdownGet(nil).Load(map[string]any{"url": "url"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
