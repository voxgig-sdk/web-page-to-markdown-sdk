...
type WebPageToMarkdownSDK struct {
	Mode     string
...
func (sdk *WebPageToMarkdownSDK) OptionsMap() map[string]any {
...
func (sdk *WebPageToMarkdownSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *WebPageToMarkdownSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *WebPageToMarkdownSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *WebPageToMarkdownSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *WebPageToMarkdownSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *WebPageToMarkdownSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("WebPageToMarkdownSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *WebPageToMarkdownSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *WebPageToMarkdownSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("WebPageToMarkdownSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *WebPageToMarkdownSDK {
	...
	sdk := NewWebPageToMarkdownSDK(sdkopts)
	sdk.Mode = "test"
