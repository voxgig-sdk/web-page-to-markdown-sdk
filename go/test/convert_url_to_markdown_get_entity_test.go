...
func TestConvertUrlToMarkdownGetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		convertUrlToMarkdownGetRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.convert_url_to_markdown_get")))
...
func convert_url_to_markdown_getBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WEB_PAGE_TO_MARKDOWN_TEST_CONVERT_URL_TO_MARKDOWN_GET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WEB_PAGE_TO_MARKDOWN_TEST_CONVERT_URL_TO_MARKDOWN_GET_ENTID"])
