...
import (
	...
	omni "github.com/voxgig-sdk/web-page-to-markdown-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WebPageToMarkdownError); is {
				out := map[string]any{
					"name":    "WebPageToMarkdownError",
...
func omniCtx(arg any, client *sdk.WebPageToMarkdownSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WebPageToMarkdownSDK); is {
			client = live
