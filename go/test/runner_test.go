...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("WEBPAGETOMARKDOWN_TEST_LIVE") == "TRUE" ||
		os.Getenv("WEBPAGETOMARKDOWN_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("WEBPAGETOMARKDOWN_TEST_EXPLAIN"); explain != "" {
