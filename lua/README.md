...
The Lua SDK for the WebPageToMarkdown API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:ConvertUrlToMarkdownGet(nil):load({ id = "example_id" }, nil)
...
local result, err = client:WebPageToMarkdown(nil):load(
  { id = "test01" }, nil
)
