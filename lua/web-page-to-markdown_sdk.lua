...
require("web-page-to-markdown_types")
...
local WebPageToMarkdownSDK = {}
WebPageToMarkdownSDK.__index = WebPageToMarkdownSDK
...
WebPageToMarkdownSDK._make_feature = _make_feature


function WebPageToMarkdownSDK.new(options)
  local self = setmetatable({}, WebPageToMarkdownSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WebPageToMarkdownSDK:options_map()
...
function WebPageToMarkdownSDK:get_utility()
  return Utility.copy(self._utility)
end


function WebPageToMarkdownSDK:get_root_ctx()
  return self._rootctx
end


function WebPageToMarkdownSDK:prepare(fetchargs)
...
function WebPageToMarkdownSDK:direct(fetchargs)
...
function WebPageToMarkdownSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function WebPageToMarkdownSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "WebPageToMarkdownSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function WebPageToMarkdownSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WebPageToMarkdownSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WebPageToMarkdownSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:ConvertUrlToMarkdownGet():list() / client:ConvertUrlToMarkdownGet():load({ id = ... })
...
-- Idiomatic facade: client:ConvertUrlToMarkdownPost():list() / client:ConvertUrlToMarkdownPost():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function WebPageToMarkdownSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WebPageToMarkdownSDK.new(sdkopts)
  sdk.mode = "test"
