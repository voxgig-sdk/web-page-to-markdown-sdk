...
// WebPageToMarkdown SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class WebPageToMarkdownTestFeature extends WebPageToMarkdownBaseFeature
{
    ...
    public function init(WebPageToMarkdownContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (WebPageToMarkdownContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = WebPageToMarkdownParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (WebPageToMarkdownContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(WebPageToMarkdownContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = WebPageToMarkdownParam::call($ctx, $k);
