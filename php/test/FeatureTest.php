...
// WebPageToMarkdown SDK feature test
//
...
class FtCtrl extends WebPageToMarkdownControl
{
...
class FtClient
{
    ...
    private WebPageToMarkdownUtility $utility;

    public function __construct(WebPageToMarkdownUtility $utility, array $options)
    {
    ...
    public function get_utility(): WebPageToMarkdownUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WebPageToMarkdownContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WebPageToMarkdownContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WebPageToMarkdownContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WebPageToMarkdownConfig::make_feature($name);
    ...
    private static function buildUrl(WebPageToMarkdownSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WebPageToMarkdownError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WebPageToMarkdownResponse($fetched);
        }
    ...
    private function fail(WebPageToMarkdownContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WebPageToMarkdownContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
