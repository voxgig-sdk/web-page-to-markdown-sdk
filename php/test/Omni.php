...
//    only - a typed WebPageToMarkdownContext there would make every ctx
...
final class WebPageToMarkdownOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new WebPageToMarkdownContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new WebPageToMarkdownSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new WebPageToMarkdownResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new WebPageToMarkdownError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new WebPageToMarkdownResponse($clean['response']);
    ...
    public static function err_from_map($m): ?WebPageToMarkdownError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WebPageToMarkdownError(is_string($code) ? $code : '', $msg);
    }
