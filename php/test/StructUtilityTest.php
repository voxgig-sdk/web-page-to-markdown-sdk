...
// WebPageToMarkdown SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = WebPageToMarkdownOmni::makeStructRunner(
    ...
    {
        return WebPageToMarkdownOmni::objmodel(WebPageToMarkdownOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return WebPageToMarkdownOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return WebPageToMarkdownOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(WebPageToMarkdownOmni::structfix($now, Struct::undef()))
                        !== json_encode(WebPageToMarkdownOmni::structfix($val, Struct::undef()))) {
