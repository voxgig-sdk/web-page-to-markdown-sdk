...
class WebPageToMarkdownSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WebPageToMarkdownHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WebPageToMarkdownHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WebPageToMarkdownFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WebPageToMarkdownFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WebPageToMarkdownUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WebPageToMarkdownHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WebPageToMarkdownHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WebPageToMarkdownHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WebPageToMarkdownError($op . "_allow",
                "WebPageToMarkdownSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WebPageToMarkdownHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WebPageToMarkdownError("graphql_error",
                "WebPageToMarkdownSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function ConvertUrlToMarkdownGet($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_convert_url_to_markdown_get === null) {
                $this->_convert_url_to_markdown_get = new ConvertUrlToMarkdownGetEntity($this, null);
            }
            return $this->_convert_url_to_markdown_get;
        }
        return new ConvertUrlToMarkdownGetEntity($this, $data);
    }
    ...
    public function ConvertUrlToMarkdownPost($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_convert_url_to_markdown_post === null) {
                $this->_convert_url_to_markdown_post = new ConvertUrlToMarkdownPostEntity($this, null);
            }
            return $this->_convert_url_to_markdown_post;
        }
        return new ConvertUrlToMarkdownPostEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new WebPageToMarkdownSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
