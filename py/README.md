...
The Python SDK for the WebPageToMarkdown API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.ConvertUrlToMarkdownGet(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.WebPageToMarkdown(None).load(
    {"id": "test01"}, None
)
