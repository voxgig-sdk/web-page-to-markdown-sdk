# WebPageToMarkdown SDK test runner
...
class WebPageToMarkdownTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WebPageToMarkdownTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WebPageToMarkdownTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WebPageToMarkdownTestRunner.getenv("WEB_PAGE_TO_MARKDOWN_TEST_LIVE")
        override = WebPageToMarkdownTestRunner.getenv("WEB_PAGE_TO_MARKDOWN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WebPageToMarkdownTestRunner.getenv(key)
        ...
        explain = WebPageToMarkdownTestRunner.getenv("WEB_PAGE_TO_MARKDOWN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WEB_PAGE_TO_MARKDOWN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WebPageToMarkdownTestRunner._test_control is not None:
            return WebPageToMarkdownTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WebPageToMarkdownTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WebPageToMarkdownTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WebPageToMarkdownTestRunner.load_test_control()
...
def load_env_local():
    WebPageToMarkdownTestRunner.load_env_local()


def env_override(m):
    return WebPageToMarkdownTestRunner.env_override(m)


def entity_data(v):
    return WebPageToMarkdownTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WebPageToMarkdownTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WebPageToMarkdownTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WebPageToMarkdownTestRunner.load_test_control()


def live_client_options():
    return WebPageToMarkdownTestRunner.live_client_options()


def live_delay_ms():
    return WebPageToMarkdownTestRunner.live_delay_ms()
