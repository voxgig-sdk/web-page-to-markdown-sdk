...
class TestConvertUrlToMarkdownGetDirect:

    def test_should_direct_load_convert_url_to_markdown_get(self):
        setup = _convert_url_to_markdown_get_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-convert_url_to_markdown_get", "live" if setup["live"] else "unit")
        if _skip:
...
def _convert_url_to_markdown_get_direct_setup(mockres):
