...
class TestConvertUrlToMarkdownGetEntity:

    def test_should_create_instance(self):
        testsdk = WebPageToMarkdownSDK.test(None, None)
        ent = testsdk.ConvertUrlToMarkdownGet(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _convert_url_to_markdown_get_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "convert_url_to_markdown_get." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WEB_PAGE_TO_MARKDOWN_TEST_CONVERT_URL_TO_MARKDOWN_GET_ENTID JSON to run live")
        client = setup["client"]
        ...
        convert_url_to_markdown_get_ref01_data = None
        if len(convert_url_to_markdown_get_ref01_data_raw) > 0:
...
def _convert_url_to_markdown_get_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/convert_url_to_markdown_get/ConvertUrlToMarkdownGetTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("WEB_PAGE_TO_MARKDOWN_TEST_CONVERT_URL_TO_MARKDOWN_GET_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("WEB_PAGE_TO_MARKDOWN_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("WEB_PAGE_TO_MARKDOWN_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("WEB_PAGE_TO_MARKDOWN_TEST_EXPLAIN") == "TRUE",
        "live": _live,
