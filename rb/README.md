# WebPageToMarkdown Ruby SDK



The Ruby SDK for the WebPageToMarkdown API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.ConvertUrlToMarkdownGet` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = WebPageToMarkdownSDK.test(testopts, sdkopts)
```
...
raise a `WebPageToMarkdownError` (a `StandardError` subclass), so wrap
...
```
rb/
├── WebPageToMarkdown_sdk.rb       -- Main SDK module
...
The main module (`WebPageToMarkdown_sdk`) exports the SDK class
