...
The Ruby SDK for the WebPageToMarkdown API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.ConvertUrlToMarkdownGet(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.WebPageToMarkdown(nil).load(
  { "id" => "test01" }, nil
)
