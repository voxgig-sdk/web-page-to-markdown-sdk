# WebPageToMarkdown Ruby SDK Reference

Complete API reference for the WebPageToMarkdown Ruby SDK.
...
#### `WebPageToMarkdownSDK.test(testopts = nil, sdkopts = nil)`
...
```ruby
result, err = client.ConvertUrlToMarkdownGet.load({ "id" => "convert_url_to_markdown_get_id" })
```
...
```ruby
result, err = client.ConvertUrlToMarkdownPost.create({
})
```
