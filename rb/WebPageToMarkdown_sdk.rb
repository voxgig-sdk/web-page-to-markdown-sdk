...
require_relative 'WebPageToMarkdown_types'
...
class WebPageToMarkdownSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WebPageToMarkdownHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WebPageToMarkdownHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WebPageToMarkdownFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WebPageToMarkdownUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WebPageToMarkdownHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WebPageToMarkdownHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WebPageToMarkdownHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WebPageToMarkdownError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue WebPageToMarkdownError => err
    ...
    if fetched.is_a?(Hash)
      status = WebPageToMarkdownHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WebPageToMarkdownError.new(
        "graphql_error", "WebPageToMarkdownSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WebPageToMarkdownSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
