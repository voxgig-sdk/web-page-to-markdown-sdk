...
class ConvertUrlToMarkdownPostEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WEB_PAGE_TO_MARKDOWN_TEST_CONVERT_URL_TO_MARKDOWN_POST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    convert_url_to_markdown_post_ref01_data = Helpers.to_map(convert_url_to_markdown_post_ref01_data_result.respond_to?(:data_get) ? convert_url_to_markdown_post_ref01_data_result.data_get : convert_url_to_markdown_post_ref01_data_result)
...
def convert_url_to_markdown_post_basic_setup(extra)
  ...
  entid_env_raw = ENV["WEB_PAGE_TO_MARKDOWN_TEST_CONVERT_URL_TO_MARKDOWN_POST_ENTID"]
