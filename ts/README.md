...
`client.ConvertUrlToMarkdownGet()` — each with a small set of operations (`load`, `create`)
...
```ts
try {
  const converturltomarkdownget = await client.ConvertUrlToMarkdownGet().load({ url: 'example_url' })
...
```ts
try {
  const converturltomarkdownget = await client.ConvertUrlToMarkdownGet().load({ url: "example" })
  console.log(converturltomarkdownget)
} catch (err) {
  console.error('load failed:', err)
}
...
| `tester(testopts?, sdkopts?)` | `WebPageToMarkdownSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `WebPageToMarkdownSDK.test(testopts?, sdkopts?)` | `WebPageToMarkdownSDK` | Create a test-mode client. |
...
| `client` | `client(): WebPageToMarkdownSDK` | Return the parent SDK client. |
...
```ts
const convert_url_to_markdown_get = await client.ConvertUrlToMarkdownGet().load({ url: 'url' })
```
...
├── src/
│   ├── WebPageToMarkdownSDK.ts        # Main SDK class
...
await converturltomarkdownget.load({ url: "example" })
