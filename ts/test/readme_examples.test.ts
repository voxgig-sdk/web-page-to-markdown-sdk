...
//     (\`new WebPageToMarkdownSDK(...)\` / \`WebPageToMarkdownSDK.test(...)\` become
//     \`WebPageToMarkdownSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WebPageToMarkdownSDK'
...
const TEST_SEED = {"entity":{"convert_url_to_markdown_get":{"test01":{"id":"test01"}},"convert_url_to_markdown_post":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WebPageToMarkdownSDK(...)\` and any
// \`WebPageToMarkdownSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WebPageToMarkdownSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WebPageToMarkdownSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
